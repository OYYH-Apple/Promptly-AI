    ...
    <Teleport to="body">
        <Transition name="fade">
            <div v-if="visible" ref="containerRef"
                ...
                @click="close" @wheel.prevent="handleWheel" @mousedown="startDrag" @mousemove="onDrag"
                @mouseup="endDrag" @mouseleave="endDrag">
                <!-- 图片显示区域 -->
                <div v-if="currentMediaType === 'image'" class="relative flex items-center justify-center"
                    :style="mediaWrapperStyle">
                    <img ref="imageRef" :src="currentMediaUrl" :alt="alt" class="rounded-lg shadow-2xl select-none"
                        :style="imageStyle" @click.stop @load="onMediaLoad" draggable="false" />
                </div>
                ...
                <!-- 缩放重置按钮（仅图片显示） -->
                <button v-if="currentMediaType === 'image'" @click.stop="resetZoom"
                ...
                <!-- 上一页按钮 -->
                <button v-if="showNavigation && currentIndex > 0" @click.stop="prev"
                ...
                <!-- 下一页按钮 -->
                <button v-if="showNavigation && currentIndex < totalMediaCount - 1" @click.stop="next"
                ...
                <!-- 底部信息栏：页码 + 类型指示器 -->
                <div v-if="showNavigation" class="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-3">
                    <!-- 媒体类型标签 -->
                    <div
                        class="px-3 py-1.5 bg-black/60 rounded-full text-white text-xs font-medium flex items-center gap-1.5">
                ...
                <!-- 图片缩放控制按钮（仅图片显示） -->
                <div v-if="currentMediaType === 'image'" class="absolute bottom-4 right-4 flex gap-2 z-10">
