  ...
  <section class="flex-1 overflow-y-auto p-10 bg-surface">
    <div class="max-w-7xl mx-auto" v-if="prompt">
      <nav class="mb-6 flex items-center gap-2 text-sm text-slate-500">
        <span @click="router.push('/')" class="hover:text-primary cursor-pointer transition-colors">{{ t('nav.library') }}</span>
      ...
      <div class="flex items-start justify-between mb-10">
        ...
        <div class="flex gap-2">
          <Tooltip :text="prompt.is_favorite ? t('tooltip.removeFromFavorites') : t('tooltip.addToFavorites')" placement="bottom">
      ...
      <div class="flex flex-col lg:flex-row gap-12">
        <div class="flex-1">
          ...
          <section v-if="prompt.reference_images?.length">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              >
                <img :src="img" :alt="t('prompt.referenceImage', { number: Number(currentImagePage) * 3 + Number(idx) + 1 })" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                <div class="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-4 flex items-end pointer-events-none">
                  <span class="text-white text-[10px] font-bold uppercase tracking-wider">{{ t('prompt.clickToView') }}</span>
                </div>
              </div>
            </div>
          </section>
        ...
        <aside class="w-full lg:w-80 space-y-8">
          <div class="bg-surface-container-low/50 border border-slate-100 rounded-2xl p-6">
            <h4 class="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-6">{{ t('metadata.title') }}</h4>
            <div class="space-y-6">
              <div>
                <label class="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">{{ t('metadata.category') }}</label>
              ...
              <div v-if="prompt.tags?.length">
                <label class="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">{{ t('metadata.tags') }}</label>
              ...
              <div>
                <label class="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">{{ t('metadata.dateCreated') }}</label>
                <p class="text-sm font-semibold text-slate-800">{{ formatDateFull(prompt.created_at) }}</p>
              </div>
              <div>
                <label class="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">{{ t('metadata.visibility') }}</label>
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm font-semibold text-slate-800">{{ prompt.is_private ? t('metadata.private') : t('metadata.public') }}</span>
                </div>
              </div>
            </div>
          </div>
